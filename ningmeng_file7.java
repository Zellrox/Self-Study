...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            // 1. Ask doctor for medical notification
            MedicalNotification n = HospitalInformation.getMedicalNotificationFromDoctor();
            handleNotification(n);

            // 2. Advance time by one hour
            for (Service s : svcs) s.tickHour();

            // 3. For any service now idle, check waiting queue
            for (Service s : svcs) {
                if (!s.isActive() && s.hasWait()) {
                    moveAndStart(s, s.nextWaitBay());
                }
            }
            ...
            // 5. Wait one hospital hour
            HospitalInformation.waitAnHour();

            // 6. Print report every 24 hours
            int h = HospitalInformation.getHourCount();
            if (h % 24 == 0) printReport();
        }
    }
    ...
    private static void handleNotification(MedicalNotification n) {
        ...
        if (!s.isActive()) moveAndStart(s, bay);
        else s.addWait(bay);
    }
    ...
    private static void printReport() {
        ...
        for (Service s : svcs)
            System.out.println(s.getName() + " in " + s.getBay()
